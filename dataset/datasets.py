...
class LIPDataSet(data.Dataset):
    def __init__(self, root, dataset, crop_size=[1024, 2014], scale_factor=0.25,
    ...
    def __getitem__(self, index):
        ...
        # parsing_anno_path = os.path.join(self.root, 'SegmentationClassAug', im_name + '.png')
        ...
        #=================================================
        h, w, _ = im.shape
        parsing_anno = np.zeros((h, w), dtype=np.int_)
        ...
        #     s = numpy.array([w,h])
        # if self.dataset == 'val':
        #     val_path = os.path.join(self.root, self.dataset + '_id.txt')
        #     val_list = [i_id.strip() for i_id in open(val_path)]
        #     im_name = val_list[index]
        #     gt_path = os.path.join(self.root,self.dataset+'_segmentations', im_name + '.png')
        #     gt = cv2.imread(gt_path, cv2.IMREAD_GRAYSCALE)


        trans = get_affine_transform(center, s, r, crop_size)
        ...
        if self.dataset != 'train':
            ...
            return input, label_parsing, hgt,wgt,hwgt, meta,mask
