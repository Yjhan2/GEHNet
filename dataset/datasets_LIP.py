...
class LIPDataSet(data.Dataset):
    def __init__(self, root, dataset, crop_size=[473, 473], scale_factor=0.25,
                 rotation_factor=30, ignore_label=255, transform=None, batch_val=False):
        """
        :rtype:
        """
        self.root = root
        ...
        self.group_list = [set1, set2, set3]
        #================================================================================
    ...
    def __getitem__(self, index):
        ...
        #=================================================
        if self.dataset != 'val' and self.batch_val is False:
            im = self.augBlock( im )
        #=================================================
        h, w, _ = im.shape
        ...
        if self.dataset != 'test' and self.batch_val is False:
            parsing_anno = cv2.imread(parsing_anno_path, cv2.IMREAD_GRAYSCALE)
        ...
        if self.dataset != 'train' and self.batch_val is False:
            return input, meta
        else:
            ...
            for index, group in enumerate(self.group_list):
                for label in group:
                    mask[label_parsing==label] = index + 1
