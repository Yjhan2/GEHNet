...
def compute_mean_ioU(preds, scales, centers, num_classes, datadir, input_size=[473, 473], dataset='val'):
    ...
    for i, im_name in enumerate(val_id):
        # gt_path = os.path.join(datadir, dataset + 'SegmentationClassAug', im_name + '.png')
        # gt_path = os.path.join(datadir, 'SegmentationClassAug', im_name + '.png')
    ...
    Avg_precision = ((tp / np.maximum(1.0, res)).mean()) * 100
    IoU_array = (tp / np.maximum(1.0, pos + res - tp))
    IoU_array = IoU_array * 100
    mean_IoU = IoU_array.mean()
    print('Pixel accuracy: %f \n' % pixel_accuracy)
    print('averange precision: %f \n' % Avg_precision)
